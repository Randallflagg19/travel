...
Миграция с Render **выполнена** 4 сентября и подтверждена пользователем.
Это архив итоговой конфигурации и отдельного ops-backlog, не текущий этап
работы. Актуальная продуктовая очередь — в [ROADMAP.md](ROADMAP.md).
...
История собственного frontend-домена и отдельного сетевого сбоя закрыта вместе
с переносом frontend на VPS. Ожидание DNS propagation и повторная настройка API
nginx не являются оставшимися шагами этой завершённой миграции.
