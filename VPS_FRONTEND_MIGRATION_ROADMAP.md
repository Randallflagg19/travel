...
- Диагностика static assets: `https://staging.tapir.su/me.png` существует и
  ...
  блокирует deploy, но требует отдельного решения по favicon и оптимизации
  крупной hero-фотографии до переключения основного домена. В инкогнито
  2026-09-08 воспроизведён `504 Gateway Timeout` у
  `/_next/image?url=/me.png...`; он оставляет белый hero-блок, а в тот же
  период может задерживать оптимизацию маскота. Это блокер пользовательского
  smoke, не считать staging готовым до исправления. Подготовлено локальное
  исправление: оригинальный `me.png` сохранён, добавлен `me-hero.jpg`
  (900×1200 JPEG, около 416 KB) и hero переведён на прямую выдачу этого файла
  без Next image optimizer. `npm run lint` и production build прошли; нужен
  ручной staging deploy и повторная проверка в чистом браузере.
- В первом browser smoke 2026-09-08 Mac получил HTML, CSS, все Next.js chunks
  ...
  `/login`/`/register` со статусом `200`. Белые экраны, для которых в
  access-log не было запросов, относятся к нестабильному VPN/browser маршруту
  до VPS, а не к frontend-коду. Функциональный smoke всё ещё требует проверки
  авторизации и операций с данными.
