...
- Workflow запускается только вручную (`workflow_dispatch`), собирает в Linux
  ...
  check. Проверка изменена на ожидание готовности до 20 секунд; требуется
  повторный зелёный запуск.
...
Повторный успешный ручной CI deploy после исправления health check и сетевой
результат переноса. Аудит ресурсов, runtime, свободный порт,
