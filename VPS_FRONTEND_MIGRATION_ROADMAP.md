...
- **Убрана лишняя кнопка mobile sidebar: реализовано локально, ждёт проверки.**
  ...
  пользователя ячейка остаётся пустой. Перед commit/deploy проверить выбор
  страны/города, включение/выключение режима, отмену удаления и отсутствие
  корзины без admin-роли.
...
- **Hero-фото по месту: реализовано локально, ждёт визуальной проверки.** В
  ...
  свою точку кадрирования. Перед commit/deploy проверить все перечисленные
  фильтры на mobile и desktop.
